...
git switch "${MERGE_INSTANCE_BRANCH}"
bazelDiff generate-hashes --bazelPath="${BAZEL_PATH}" --workspacePath="$(pwd)" "-so=${bazel_startup_options}" "${merge_instance_branch_out}"
...
bazelDiff generate-hashes --bazelPath="${BAZEL_PATH}" --workspacePath="$(pwd)" "-so=${bazel_startup_options}" "${merge_instance_with_pr_branch_out}"
